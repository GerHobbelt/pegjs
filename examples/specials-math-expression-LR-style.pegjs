/*
 * Math expression grammar ripped from the JavaScript Grammar
 * 
 * This one is constructed in LR, i.e. (!)left-recursive fashion, suitable for PEG parser generators
 * which *do* support (both indirect & direct!) left recursion. Note that precedence is encoded in the
 * order of the main choiceset in the Expression rule (which results in a shallow stack call depth for *any*
 * math expression!) while the associativity has to be dealt with via the annotations.
 */
...
Start
  = __ program:Expression __ { return program; }
