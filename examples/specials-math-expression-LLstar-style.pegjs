 ...
 * 
 * This one is constructed in LL(*) fashion, suitable for PEG parser generators
 * which do not support left recursion. Note that precedence is encoded in the
 * order of the rules (which results in a *huge* stack call depth for *any*
 * math expression!) and the associativity has to be dealt with in the user
 * action code, making this a non-trivial grammar definition.
 */
