var utils = {
  ...
  findAnnotationsByName: function(rule, name) {
    return utils.filter(rule.annotations, function(a) { return a.name === name; });
  },
  ...
  globalFindAnnotationsByName: function(rules, name) {
    var rv = [];
    utils.each(rules, function(rule, index) {
      rv.concat(utils.filter(rule.annotations, function(a) { return a.name === name; }));
    });
    return rv;
  },
  ...
  globalApplyAnnotationConditionally: function(rules, annotation, annotation_names) {
    if (annotation_names == null) {
      ...
      annotation_names = [];
    } else if (annotation_names.length == null) {
      // assume a single name was passed
      annotation_names = [annotation_names];
    }
    utils.each(rules, function(rule, index) {
      if (!utils.find(rule.annotations, function(a) { return annotation_names.indexOf(a.name) >= 0; })) {
        // none of the 'blocking' annotation_names exist for this rule, hnce we apply the annotation to it:
        if (!rule.annotations) {
          rule.annotations = [];
        }
