...
module.exports = function(ast, options) {
  ...
  function generateTables() {
    ...
    if (options.optimize === "size") {
      return [
        ...
        '],',
        'peg$actions = [',
           indent2(utils.map(ast.actions, createFunc).join(',\n')),
        '],',
        '',
        'peg$bytecode = [',
           indent2(utils.map(
  ...
  function generateCacheHeader(ruleIndexCode) {
    return [
      ...
      (options.collectRunTimeStatistics ? '  peg$cache_use_counters[' + ruleIndexCode + ']++;' : ''),
  ...
  function generateRuleFunction(rule) {
    ...
    parts.push([
      ...
      '  var peg$currentRuleIndex = ' + utils.indexOfRuleByName(ast, rule.name) + ';',
      '  var ' + utils.map(utils.range(0, stack.maxSp + 1), s).join(', ') + ';',
      ''
    ].join('\n'));

    var hasNoCacheAnnotation = utils.findAnnotationByName(rule, "nocache");
  ...
  var has_cacheAllNone = utils.find(ast.rules, function (r) {
    var a = utils.find(r.annotations, function (a) {
  ...
  if (has_cacheAllNone) {
    has_cacheAllNone = utils.find(has_cacheAllNone.annotations, function (a) { return anno_names.indexOf(a.name) >= 0; });
    var param = utils.find(has_cacheAllNone.params, function (a) { return a === 'all' || a === 'none'; });
    param = (param === 'none');
  ...
  utils.globalApplyAnnotationConditionally(ast.rules, {
    type: 'annotation',
  ...
  if (utils.globalFindAnnotationsByName(ast.rules, 'cache')) {
    options.cache = 1;
  }

  // and also see whether we have a `@collect_statistics` annotation somewhere...
  if (utils.globalFindAnnotationsByName(ast.rules, 'collect_statistics')) {
  ...
  if (options.collectRunTimeStatistics || options.includeRuleNames) {
    ...
    utils.each(ast.rules, function(rule, index) {
      parts.push('        "' + rule.name + '",             // index: ' + index);
    });
    parts.push([
      '      ];',
      '',
      '  function getRuleNamesIndexTable() {',
  ...
  if (options.collectRunTimeStatistics) {
    var initparts = ['  var peg$cache_use_counters = ['];
    utils.each(ast.rules, function(rule, index) {
      initparts.push(' 0,');
    });
    initparts.push('];');
    parts.push(initparts.join(""));
    parts.push([
      '',
      '  function getStatistics(reset) {',
      '    var rv = {',
  ...
  parts.push([
    ...
    '      var index = index == null ? peg$currentRule : index;',
  ...
  parts.push([
    ...
    (options.collectRunTimeStatistics ? '    getStatistics:          getStatistics,' : ''),
    '    getRuleNamesIndexTable: getRuleNamesIndexTable,',
    '    parse:                  parse',
    '  };',
    '})()'
  ].join('\n'));
