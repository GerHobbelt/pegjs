...
module.exports = function(ast, options) {
  ...
  function generateInterpreter() {
    ...
    var do_cache = (options.cache && !utils.findAnnotationByName(rule, "nocache"));
  ...
  var has_cacheAllNone = utils.find(ast.rules, function (r) { 
    if (anno_names.indexOf(r.name) >= 0) {
      return r.params.indexOf('all') > 0 || r.params.indexOf('none') > 0; 
    }
    return false;
  });
  var caching = option.cache;
  if (has_cacheAllNone) {
