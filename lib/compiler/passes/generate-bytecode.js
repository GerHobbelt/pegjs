...
function generateBytecode(ast) {
  ...
  function buildRangeMax(max, bodyCode, env, sp) {
    if (max && max.value) {
      var checkCode = max.constant
        ? [op.IF_GE, max.value]
        : [op.IF_GE_DYNAMIC, sp - env[max.value]];

      return buildCondition(//   if (result.length >= max) {
  ...
  var generate = visitor.build({
    ...
    range: function(node, context) {
      var expressionCode = generate(node.expression, {
          ...
          checkMaxCode   = buildRangeMax(node.max, bodyCode, context.env, context.sp),
