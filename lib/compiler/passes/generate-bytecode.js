...
function generateBytecode(ast) {
  ...
  function buildRangeMax(max, bodyCode, env, sp, hasMin, onlyForDynamic) {
    if (max && max.value) {
      var checkCode = max.constant
        ? [op.IF_GE, max.value]
        : [op.IF_GE_DYNAMIC, sp - env[max.value] + 1 + (hasMin ? 1 : 0)];

      return onlyForDynamic && max.constant
        ? bodyCode
        : buildCondition(     //   if (result.length >= max) {
          checkCode,          //                          stack:[ [elem...] ]
  ...
  function buildRangeMin(min, mainLoopCode, env, sp) {
    if (min && min.value) {
      var checkCode = min.constant
        ? [op.IF_LT, min.value]
        : [op.IF_LT_DYNAMIC, sp - env[min.value] + 2];

      // If non-constant or constant not |0|, then add restriction.
      // If constat is |0|, then restriction not needed.
      return !min.constant || min.value > 0 ?
        buildSequence(
  ...
  function buildRangeDelimiter(delimiterNode, expressionCode, context) {
    return delimiterNode !== null ?
      buildSequence(                  //                          stack:[  ]
  ...
  var generate = visitor.build({
    ...
    range: function(node, context) {
      var expressionCode = generate(node.expression, {
          ...
          hasMin         = node.min !== null && (!node.min.constant || node.min.value > 0),
          bodyCode       = buildRangeDelimiter(node.delimiter, expressionCode, context),
          // Check high boundary, if it defined.
          checkMaxCode   = buildRangeMax(node.max, bodyCode, context.env, context.sp, hasMin, false),
          mainLoopCode   = buildSequence(
            ...
            buildRangeMax(node.max,
              expressionCode, context.env,    // var elem = expr();       stack:[ pos, [], elem ]
              context.sp, hasMin, true
            ),
      ...
      return buildRangeMin(node.min, mainLoopCode, context.env, context.sp);
    },
