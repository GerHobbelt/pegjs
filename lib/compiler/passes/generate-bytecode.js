...
function generateBytecode(ast) {
  ...
  function buildRangeMax(max, bodyCode) {
    return max !== null ?
      buildCondition(                 //   if (result.length >= max) {
        [op.IF_ARRLEN_MAX, max],      //                          stack:[ [elem...] ]
  ...
  function buildRangeMin(minNode, mainLoopCode) {
    return min !== null && min > 0 ?
      buildSequence(
        mainLoopCode,                   // result = [elem...];      stack:[ pos, [elem...] ]
        buildCondition(
          [op.IF_ARRLEN_MIN, min],      // if (result.length < min) {
  ...
  var generate = visitor.build({
    ...
    range: function(node, context) {
      var expressionCode = generate(node.expression, {
          ...
          checkMaxCode   = buildRangeMax(node.max, bodyCode),
      ...
      return buildRangeMin(node.min, mainLoopCode);
    },
