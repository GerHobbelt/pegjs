...
function generateBytecode(ast) {
  ...
  function addConst(type, value) {
    ...
    //console.error('addConst: ', index, type, value, s);
  ...
  var generate = visitor.build({
    grammar: function(node, parent) {
      //console.error("FIRST const list dump: ", consts);
      arrays.each(node.rules, function (rule) {
        generate(rule, node);
      });

      //console.error("SECOND const list dump: ", consts);
      node.consts = consts;
    ...
    unlabeled: function(node, parent, context) {
      var env = objects.clone(context.env);
    ...
    code: function (node, parent, context) {
      ...
      //console.error("process CODE chunk as CONST: ", node, " --> ", nodeDescription, " :: parent: ", parent);      
      var expectedIndex = addConst('expected', [
        '{',
        'type: "code",',
