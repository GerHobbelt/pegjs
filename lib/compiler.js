...
var compiler = {
  ...
  passes: {
    ...
    transform: {
      ...
      autoLabel:             require("./compiler/passes/auto-label"),
      calculateConsumption:  require("./compiler/passes/calculate-consumption-info"),
  ...
  compile: function(ast, passes) {
    ...
    var options = arguments.length > 2 ? objects.clone(arguments[2]) : {},
        ...
        collector = {
          ...
          emitError: function(message, extra_info) {
            ++errors;
            problems.push(['error', message, extra_info]);
          },
          emitWarning: function(message, extra_info) {
            problems.push(['warning', message, extra_info]);
          },
          emitInfo: function(message, extra_info) {
            problems.push(['info', message, extra_info]);
          }
        };
    ...
    if (arrays.contains(options.allowedStartRules, "*")) {
      ...
      options.allowedStartRules = ast.rules.map(function(rule) { 
        return rule.name; 
      });
    }
    ...
    for (stage in passes) {
      if (passes.hasOwnProperty(stage)) {
        ...
        if (errors !== 0) {
          throw new GrammarError("Stage " + stage + " contains errors.", { problems: problems });
        }
        else if (options.verboseOutput && problems.length > 0) {
          console.log("Stage " + stage + ":\n");
          console.log(GrammarError.buildMessage(null, problems).join('\n'));
        }
      }
    }
